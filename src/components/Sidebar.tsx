...
const Sidebar = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 h-full bg-white shadow-lg transition-all z-50 top-16 md:top-16 ${isOpen ? "w-64" : "w-0 md:w-64"
...
const SidebarItem = ({ to, icon, text, onClose }: { to: string; icon: JSX.Element; text: string; onClose: () => void }) => {
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-3 p-2 rounded-md transition-all ${isActive ? "bg-blue-500 text-white font-semibold" : "hover:bg-gray-100 text-gray-900"
        }`}
    >
      {icon}
